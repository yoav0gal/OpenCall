...
async function canReachBridge() {
  try {
    await fetchJson<HealthPayload>(`${getBridgeUrl().replace(/\/$/, "")}/health`);
    return true;
  } catch {
    return false;
  }
...
async function startBridge() {
  ...
  const command = [
    "bash",
    "-lc",
    `nohup bash -lc 'cd ${bridgeDir} && export OPENCALL_HOME=${JSON.stringify(opencallHome)} && export $(cat ${JSON.stringify(envPath)} | xargs) && exec bun run src/index.ts' >> ${JSON.stringify(logPath)} 2>&1 & echo $!`
  ];
...
async function statusBridge() {
  ...
  try {
    const health = await fetchJson<HealthPayload>(`${getBridgeUrl().replace(/\/$/, "")}/health`);
